...
function getqrpic(force){
	...
	if(qrsig!=null && qrimg!=null && force==false){
		...
		var getvcurl='qq/getsid/login.php?do=getqrpic&r='+Math.random(1);
...
function qrlogin(){
	...
	var url = 'qq/getsid/login.php?do=qrlogin&findpwd=1&qrsig='+decodeURIComponent(qrsig)+'&r='+Math.random(1);
	$.get(url, function(d) {
		if(d.saveOK ==0){
