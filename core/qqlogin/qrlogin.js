...
function getqrpic(force){
	...
	if(qrsig!=null && qrimg!=null && force==false){
		...
		var getvcurl=siteurl+'qq/getsid/login.php?do=getqrpic&r='+Math.random(1);
...
function qrlogin(){
	...
	var url = siteurl+'qq/getsid/login.php?do=qrlogin&qrsig='+decodeURIComponent(qrsig)+'&r='+Math.random(1);
	$.get(url, function(d) {
		if(d.saveOK ==0){
			...
			cleartime();
			ajax.post("ajax.php?mod=addqq&act=add&r="+Math.random(1),{qq:d.uin, skey:d.skey, pskey:d.pskey, superkey:d.superkey}, 'json', function(arr) {
				if(arr.code==1) {
					alert(arr.msg);
					showlist('qqlist',1);
				}else{
