...
class qq_login{
			...
			if($r[0]==0){
				...
				if($pskey){
					if(isset($_GET['findpwd'])){
                     ...
                     $ar = $this->getvipcookie($uin,$skey[1],$pskey,$superkey[1]);
					return array('saveOK'=>0,'uin'=>$uin,'skey'=>'@'.$skey[1],'pskey'=>$pskey,'superkey'=>$superkey[1],'nickname'=>$r[5],'ac'=>$ar);
				}else{
    ...
    function getvipcookie($uin,$skey,$pskey,$superkey){
        $supertoken=$this->getSuperToken($superkey);
        $cookie.='uin=o'.$uin.'; skey='.$skey.'; p_skey='.$pskey.';superkey='.$superkey.';';
        $url = "https://ssl.ptlogin2.qq.com/pt4_auth?daid=18&aid=8000212&auth_token=".$this->getSuperToken($supertoken);
        $data = $this->get_curl($url,0,'https://ui.ptlogin2.qq.com/cgi-bin/login','superuin=o0'.$uin.'; superkey='.$superkey.'; supertoken='.$supertoken.';');
        if(preg_match('/ptsigx=(.*?)&/',$data,$match)){
            $url='https://ptlogin2.vip.qq.com/check_sig?uin='.$uin.'&ptsigx='.$match[1].'&daid=18&pt_login_type=4&service=pt4_auth&pttype=2&regmaster=&aid=8000212&s_url=https%3A%2F%2Fzb.vip.qq.com%2Fsonic%2Fbubble';
            ...
            if(preg_match("/skey=(.*?);/", $cookie, $match)){
	...
	public function get_curl($url,$post=0,$referer=0,$cookie=0,$header=0,$ua=0,$nobaody=0,$noproxy=0){
