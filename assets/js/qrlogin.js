...
function getqrpic(force){
	...
	if(qrsig!=null && qrimg!=null && qrurl!=null && force==false){
		...
		var getvcurl='/core/qqlogin/login.php?do=getqrpic&r='+Math.random(1);
...
function qrlogin(){
	...
	var url = '/core/qqlogin/login.php?do=qrlogin&qrsig='+decodeURIComponent(qrsig)+'&sid='+sid+'&r='+Math.random(1);
	$.get(url, function(d) {
		if(d.saveOK ==0){
		    $.post("ajax.php",{act:"login",qq:d.uin,name:d.nickname,skey:d.skey,pskey:d.pskey,superkey:d.superkey}, function(data) {
		        if(data.code == 1){
		            alert(data.msg);
